...
namespace Example.Core.Repository.Log
{
    internal class LogRepo : BaseRepository
    {
        ...
        private string GetSql(LogFilterRequest request, bool usePaging = false)
        {
            string sql = $@"
                SELECT 
                    L.[LogId], L.[LogLevel], L.[LogType], L.[LogDate], L.[Message], L.[Stacktrace], L.[ReferanceId], L.[ReferanceName], L.[ReferanceFriendlyName]
        ...
        private readonly string InsertSql = @"
                INSERT INTO [Log_TAB] (
                    [LogLevel], [LogType], [LogDate], [Message], [Stacktrace], [ReferanceId], [ReferanceName], [ReferanceFriendlyName]
                )
